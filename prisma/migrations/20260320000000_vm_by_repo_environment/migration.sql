...
-- Update vm_approval_requests table
ALTER TABLE `vm_approval_requests` ADD COLUMN `environment` VARCHAR(50) NULL;

-- Backfill environment from metadata for existing approval requests
UPDATE `vm_approval_requests` SET `environment` = JSON_UNQUOTE(JSON_EXTRACT(`metadata`, '$.environment')) WHERE `metadata` IS NOT NULL AND JSON_EXTRACT(`metadata`, '$.environment') IS NOT NULL;

-- Set default environment for any remaining approval requests
UPDATE `vm_approval_requests` SET `environment` = 'dev' WHERE `environment` IS NULL;

-- Make environment field required
ALTER TABLE `vm_approval_requests` MODIFY `environment` VARCHAR(50) NOT NULL;
